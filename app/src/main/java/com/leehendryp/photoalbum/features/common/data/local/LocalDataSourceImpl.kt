...
import com.leehendryp.photoalbum.core.domain.DomainException
import com.leehendryp.photoalbum.core.domain.INSERTION_ERROR
import com.leehendryp.photoalbum.core.domain.NO_SUCH_ITEM_ERROR
...
import com.leehendryp.photoalbum.features.common.data.local.model.toLocalDataModels
...
internal class LocalDataSourceImpl @Inject constructor(
    ...
    override fun getPagedAlbums(): PagingSource<Int, AlbumEntity> = try {
        ...
        throw DomainException.ClientException(RETRIEVAL_ERROR, cause)
    }

    override suspend fun save(albums: List<Album>) = try {
        dao.insertAll(albums.toLocalDataModels())
    } catch (cause: Throwable) {
        throw DomainException.ClientException(INSERTION_ERROR, cause)
    }

    override suspend fun getAlbum(id: Int): AlbumEntity = try {
        dao.getAlbum(id)
    } catch (cause: Throwable) {
        throw DomainException.ClientException(NO_SUCH_ITEM_ERROR, cause)
    }
}
